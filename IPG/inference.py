...
def log_validation(
    ...
    pose_paths = [
        os.path.join(pose_dir, x)
        for x in os.listdir(pose_dir)
        if os.path.isfile(os.path.join(pose_dir, x))
        and os.path.splitext(x)[1].lower() in valid_exts
    ]
    pose_paths.sort()
    print(f"[INFO] Found {len(pose_paths)} valid pose images in {pose_dir}")
    ...
    for i in range(len(img_list)):
        ...
        for j in range(len(pose_paths)):
            ...
            # 修改
            pose_img = cv2.imread(pose_path_)
            if pose_img is None:
                print(f"[WARN] Failed to read pose image: {pose_path_}")
                continue
            pose_image_pil = Image.fromarray(cv2.cvtColor(pose_img, cv2.COLOR_BGR2RGB))
