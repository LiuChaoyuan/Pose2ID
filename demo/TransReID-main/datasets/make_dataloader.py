...
def val_collate_fn(batch):
    imgs, pids, camids, viewids, img_paths, imgs_ipg = zip(*batch)
    viewids = torch.tensor(viewids, dtype=torch.int64)
    camids_batch = torch.tensor(camids, dtype=torch.int64)

    imgs_ipg_list = []
    for i in range(len(imgs_ipg[0])):
        ipgs = []
        for j in range(len(imgs)):
            ipgs.append(imgs_ipg[j][i])
        imgs_ipg_list.append(torch.stack(ipgs, dim=0))
        
    return torch.stack(imgs, dim=0), pids, camids, camids_batch, viewids, img_paths, imgs_ipg_list
